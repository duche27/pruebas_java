...
public class Uso_taxis {
    ...
    public static <arr> void main(String[] args) {
        ...
        intList.add(256741038);
        intList.add(623958417);
        intList.add(467905213);
        intList.add(714532089);
        intList.add(938071625);

        System.out.println(isNumberIncluded(intList, 40));
        System.out.println(oddNumber(2, 9));
        plusMinus(intList);
        miniMaxSum(intList);
        
//        long result = getMaxAdditionalDinersCountWithSideSocialDistance(10, 1, 2,
//                new long[] { 2, 6 });
//        long result2 = getMaxAdditionalDinersCountWithoutSideSocialDistance(10, 1, 2,
//                new long[] { 2, 6 });
...
//
//        // GROUPING + REDUCING
//        Map<String, Optional<Factura>> collect = facturas.stream().filter(f -> f.getImporte() > 10)
//                .collect(Collectors.groupingBy(Factura::getConcepto, Collectors.reducing(
//                        BinaryOperator.maxBy(Comparator.comparing(Factura::getImporte)))));
//
//        // COUNT
...
//        list.forEach(System.out::println);
    ...
    private static void plusMinus(List<Integer> intList) {
        int positive = 0;
        int negative = 0;
        int zero = 0;
        ...
        System.out.format("%.6f",(double) positive / intList.size());
        System.out.format("\n%.6f",(double) negative / intList.size());
        System.out.format("\n%.6f",(double) zero / intList.size());
    }
    
    private static List<Integer> oddNumber(int i, int i1) {
        List<Integer> intOddNumbersList = new ArrayList<>();
        for (int p = i; p <= i1; p++) if (p % 2 != 0) intOddNumbersList.add(p);
        return intOddNumbersList;
    }
    
    private static String isNumberIncluded(List<Integer> intList, int i) {
        int rest = intList.stream().filter(e -> e == i).findFirst().orElse(0);
        return rest == 0 ? "NO": "YES";
    }
}
